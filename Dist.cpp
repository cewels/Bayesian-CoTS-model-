...
double Context::lognormal_dis(double mean, double sigma, RandomGeneratorType& rg) {
	std::lognormal_distribution<> distribution(mean, sigma);
	boost::variate_generator< RandomGeneratorType&, std::lognormal_distribution<> >
		log_s(rg, distribution);
	distribution.reset();
	return log_s();
}

long long Context::normal_dis_long(long long mean, long long S, RandomGeneratorType& rg) {
	boost::normal_distribution<> distribution(mean, S);
	boost::variate_generator< RandomGeneratorType&, boost::normal_distribution<> >
		norm_s(rg, distribution);
	distribution.reset();
